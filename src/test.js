// 本试卷唯一 ID: 7001B08D00B16861E541055B97337535, 请勿修改本行内容
/**
...
module.exports = {
	...
	qn15: () => {
		...
		// B. Reducer中不能做异步操作
		//
		// C. Redux store可以从js文件export并在任意位置使用
		//
		// D. Redux用于应用的状态管理
		return 'A';
	},
	...
	frequencyMap: strs => {
		...
		strs.map(prod=>{
			result = {
				...result,
				[prod]: result[prod] ? ++result[prod] : 1
			};
		});
		return result;
	},
	...
	decode: str => {
		...
		for(let i=0 ;i<strArr.length;i++){ //循环数组并把首字母改成大写并拼接
			let reg = /^[?]/g; // 匹配首位是否是?号
	...
	minRequiredMeetingsRooms: meetings => {
		...
		meetings.map((prod)=>{
			start = [...start, prod[0]].sort((a, b)=>a-b);
			end = [...end, prod[1]].sort((a, b)=>a-b);
		}, []);
		start.map(prod=>{
			if(end[0] <= prod){
				end.splice(0, 1);
